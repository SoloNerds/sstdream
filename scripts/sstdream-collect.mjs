...
//
// This file is SELF-CONTAINED on purpose: download just this one file and run it.
// (The sanitizer below is the canonical scripts/sanitize.mjs, inlined; a test keeps
// the two byte-for-byte in sync, and another actually runs this script end-to-end.)
//
...
// ===== sanitize — inlined canonical copy of scripts/sanitize.mjs =========================
// Strips secret VALUES from pasted SST infra while preserving the structural
// `new sst.aws.X("Name", {...})` calls the diagram needs. Philosophy: OVER-REDACT.
// Blanking a non-secret is cosmetic; leaking a real one into a browser paste is a breach.
// These consts MUST be declared before main() is invoked (at the bottom) — declaring the
// run-guard last keeps them out of the temporal dead zone.
...
export function sanitize(input) {
  ...
  // 5) Name heuristic: a secret-named field assigned a string / concat / array value.
...
// Run main() only when executed directly (not when imported by a test). Declared LAST so
// every const above is initialized before main() touches the sanitizer — no dead-zone.
